...
class Handlebars {
  ...
  Handlebars({bool escapeHtml = true}) : _escapeHtml = escapeHtml {
  ...
  ///
  /// The returned function accepts a context object and returns
  /// the rendered string.
  ///
  /// ```dart
  ...
  /// ```
  String Function(dynamic context) compile(String source) {
  ...
  void _ensurePartialsCompiled() {
    for (final entry in _partialSources.entries) {
      if (!_compiledPartials.containsKey(entry.key)) {
        ...
        _compiledPartials[entry.key] = (context) => template(context);
      }
    }
  }
