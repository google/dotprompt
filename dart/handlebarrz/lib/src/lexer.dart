...
class Lexer {
  ...
  Token _readText() {
    ...
    while (_pos < source.length) {
      ...
      // TODO(https://github.com/google/dotprompt/issues/999): Escape sequence
      // detection has caching issues with Bazel - needs investigation.
  ...
  Token _readHandlebarsOpen() {
    ...
    // Raw block {{{{ - must be checked before {{#
    ...
    if (_lookingAt("{{{{")) {
      ...
      final contentStartLine = _line;
      final contentStartColumn = _column;
      ...
      // Return multiple tokens for raw block:
      // We'll add them to the token stream in the main tokenize method
      // For now, return a special combined token
      rawBlockName = name;
