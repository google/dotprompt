  ...
  group("Handlebars", () {
    ...
    group("helpers", () {
      ...
      test("helper with string literal argument", () {
        hb.registerHelper("greet", (args, options) {
          return "Hello, ${args[0]}!";
        });
      ...
      test("helper with hash arguments", () {
        ...
        expect(
          ...
          equals('<a href="https://example.com">Go</a>'),
        );
      });
    ...
    group("block helpers", () {
      ...
      test("each block with else", () {
        final template = hb.compile(
          "{{#each items}}{{.}}{{else}}empty{{/each}}",
        );
        expect(template({"items": []}), equals("empty"));
        expect(template({"items": ["a"]}), equals("a"));
      });
    });
    ...
    group("custom block helpers", () {
      ...
      test("unlessEquals helper", () {
        ...
        final template = hb.compile(
          '{{#unlessEquals status "active"}}Not Active{{else}}Active{{/unlessEquals}}',
        );
    ...
    group("dotprompt helpers", () {
      test("role helper", () {
        hb.registerHelper("role", (args, options) {
          return SafeString("<<<dotprompt:role:${args[0]}>>>");
        });

        final template = hb.compile('{{role "system"}}This is system');
        expect(
          template(<String, dynamic>{}),
          equals("<<<dotprompt:role:system>>>This is system"),
        );
      });
      ...
      test("history helper", () {
        ...
        expect(
          template(<String, dynamic>{}),
          equals("Before <<<dotprompt:history>>> After"),
        );
      });
      ...
      test("media helper with hash args", () {
        hb.registerHelper("media", (args, options) {
          final url = options.hash["url"];
          final contentType = options.hash["contentType"] ?? "";
          return SafeString(
      ...
      test("section helper", () {
        hb.registerHelper("section", (args, options) {
          return SafeString("<<<dotprompt:section ${args[0]}>>>");
        });

        final template = hb.compile('{{section "code"}}');
        expect(
          template(<String, dynamic>{}),
          equals("<<<dotprompt:section code>>>"),
        );
      });

      test("json helper", () {
        hb.registerHelper("json", (args, options) {
          final indent = options.hash["indent"] as int? ?? 0;
          // Simple JSON - in real impl would use dart:convert
          return SafeString(args[0].toString());
        });
    ...
    group("escape sequences", () {
      test(r"backslash-brace outputs literal braces", () {
        // \{{ should output literal {{
        final source = r"Show \{{name}} literally";
        final template = hb.compile(source);
        expect(template({"name": "World"}), equals("Show {{name}} literally"));
      });

      test(r"double backslash before variable is literal", () {
    ...
    group("subexpressions", () {
      test("basic subexpression", () {
        ...
        hb.registerHelper("wrap", (args, options) {
          return "[${args[0]}]";
        });
      ...
      test("nested subexpressions", () {
        hb.registerHelper("add", (args, options) {
          return (args[0] as num) + (args[1] as num);
        });
        hb.registerHelper("mult", (args, options) {
          return (args[0] as num) * (args[1] as num);
        });
      ...
      test("subexpression with hash args", () {
        hb.registerHelper("format", (args, options) {
    ...
    group("data variables", () {
      test("@root accesses root context", () {
        final template = hb.compile(
          "{{#with user}}{{name}} from {{@root.company}}{{/with}}",
        );
        expect(
          template({
            "user": {"name": "Alice"},
      ...
      test("@index in nested each", () {
        final template = hb.compile(
          "{{#each outer}}{{@index}}:[{{#each .}}{{@index}}{{/each}}]{{/each}}",
        );
        expect(
          template({
            "outer": [
    ...
    group("parent context", () {
      ...
      test("../value in each loop with primitive values", () {
        final template = hb.compile(
          "{{#each items}}Item {{.}} from {{../source}}; {{/each}}",
        );
        expect(
          template({
            "source": "data",
    ...
    group("block params", () {
      ...
      test("each over object with block params - value and key", () {
        final template = hb.compile(
          "{{#each obj as |val key|}}{{key}}={{val}};{{/each}}",
        );
        expect(
          template({
            "obj": {"x": 1, "y": 2},
    ...
    group("raw blocks", () {
      // TODO(#999): Raw blocks need deeper investigation into lexer/parser interaction
      test("raw block outputs content literally", () {
        final template = hb.compile(
          "Before {{{{raw}}}}{{name}} is literal{{{{/raw}}}} After",
        );
        expect(
          template({"name": "World"}),
          equals("Before {{name}} is literal After"),
        );
      }, skip: "Raw blocks requires parser refactoring");
    });
    ...
    group("partial blocks", () {
      test("partial block uses partial if found", () {
        ...
        final template = hb.compile(
          "{{#> myPartial}}Default{{/myPartial}}",
        );
        expect(
          template(<String, dynamic>{}),
          equals("PARTIAL CONTENT"),
        );
      });

      test("partial block uses default content if partial not found", () {
        final template = hb.compile(
          "{{#> missingPartial}}Default Content{{/missingPartial}}",
        );
        expect(
          template(<String, dynamic>{}),
          equals("Default Content"),
        );
      });
    });
    ...
    group("literals", () {
      test("boolean literal true", () {
        hb.registerHelper("showBool", (args, options) {
          return args[0] == true ? "yes" : "no";
        });
    ...
    group("edge cases", () {
      ...
      test("empty array is falsy in if", () {
        ...
        expect(template({"items": []}), equals("no"));
        expect(template({"items": [1]}), equals("yes"));
      });
    });
  });
}
