...
void main() {
  // Get spec file path from environment variable
  final specFile = Platform.environment["SPEC_FILE"];
  if (specFile == null || specFile.isEmpty) {
    fail("SPEC_FILE environment variable not set");
    return;
  }

  // Find the workspace root
  final workspaceRoot = _findWorkspaceRoot();
  final specPath = "$workspaceRoot/$specFile";
  ...
  for (final spec in specs) {
    ...
    final resolverPartials =
        specMap["resolverPartials"] as Map<String, dynamic>?;
      ...
      for (final testCase in tests) {
        ...
        test(desc, () async {
          ...
          final dotpromptOptions = DotpromptOptions(
            partials: {
              ...?partials?.cast<String, String>(),
            },
            partialResolver: resolverPartials != null
                ? (name) async => resolverPartials[name] as String?
                : null,
          );
          ...
          if (expected.containsKey("messages")) {
            ...
            for (var i = 0; i < expectedMessages.length; i++) {
              final expectedMsg =
                  expectedMessages[i] as Map<String, dynamic>;
              final actualMsg = result.messages[i];

              // Check role
              if (expectedMsg.containsKey("role")) {
                expect(
                  actualMsg.role.value,
                  equals(expectedMsg["role"]),
                );
              }
              ...
              if (expectedMsg.containsKey("content")) {
                ...
                for (var j = 0; j < expectedContent.length; j++) {
                  final expectedPart =
                      expectedContent[j] as Map<String, dynamic>;
                  ...
                  if (expectedPart.containsKey("text")) {
                    ...
                    final expectedMedia =
                        expectedPart["media"] as Map<String, dynamic>;
                    expect(
                      actualMedia.url,
                      equals(expectedMedia["url"]),
                    );
