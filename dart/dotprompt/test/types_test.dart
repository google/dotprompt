...
void main() {
  ...
  group("DataArgument", () {
    ...
    test("serializes to JSON", () {
      ...
      expect(json["input"]["name"], equals("Alice"));
    });
  ...
  group("ContextData", () {
    ...
    test("accesses additional data", () {
      final context = ContextData.fromJson({
        "state": {"count": 42},
        "auth": {"email": "test@example.com"},
      });
      expect(context["auth"]?["email"], equals("test@example.com"));
    });

    test("serializes to JSON", () {
      const context = ContextData(
        state: {"count": 42},
        data: {"auth": {"email": "test@example.com"}},
      );
      final json = context.toJson();
      expect(json["state"]["count"], equals(42));
      expect(json["auth"]["email"], equals("test@example.com"));
    });
  });
  ...
  group("Message", () {
    ...
    test("serializes to JSON", () {
      ...
      expect(json["content"][0]["text"], equals("Hello!"));
    });
  ...
  group("Part", () {
    ...
    group("MediaPart", () {
      ...
      test("serializes to JSON", () {
        ...
        expect(json["media"]["url"], equals("image.png"));
        expect(json["media"]["contentType"], equals("image/png"));
      });
    ...
    group("ToolResponsePart", () {
      test("creates with tool response", () {
        ...
        expect(part.toolResponse.output, isA<Map>());
      });
    });
  });
}
