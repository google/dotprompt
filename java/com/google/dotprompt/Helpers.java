...
public class Helpers {
  ...
  /**
   * Registers all standard Dotprompt helpers with the given instance.
   *
   * @param dotprompt The Dotprompt instance to register helpers with.
   */
  public static void register(Dotprompt dotprompt) {
    dotprompt.registerHelper("json", Helpers::json);
    dotprompt.registerHelper("media", Helpers::media);
    dotprompt.registerHelper("role", Helpers::role);
    dotprompt.registerHelper("history", Helpers::history);
    dotprompt.registerHelper("section", Helpers::section);
    dotprompt.registerHelper("ifEquals", Helpers::ifEquals);
    dotprompt.registerHelper("unlessEquals", Helpers::unlessEquals);
  }
  ...
  public static Object json(Object context, Options options) throws IOException {
    ...
    // If param is 0, context is used. JKnack passes arg as context for single-arg calls?
    // If context is the root data (Map) and we wanted that, fine.
    // If context is "argument", fine.
  ...
  public static Object ifEquals(Object context, Options options) throws IOException {
    ...
    if (options.params.length == 0) {
      // Arguments might be (Context, ?) -> missing 2nd arg?
      // Or (Context=arg0), params empty. Missing arg1.
      return options.inverse(context);
    } else if (options.params.length == 1) {
      // context=arg0, param(0)=arg1
      arg0 = context;
      arg1 = options.param(0);
    } else {
      // param(0)=arg0, param(1)=arg1
