...
describe('splitByRoleAndHistoryMarkers', () => {
  ...
  it('splits a string with a single marker correctly', () => {
    ...
    // When splitting, the marker "<<<dotprompt:role:assistant>>>" is captured
    // as "<<<dotprompt:role:assistant" so the result is an array of: [ 'Hello
    // ', '<<<dotprompt:role:assistant', ' world' ]
    const output = splitByRoleAndHistoryMarkers(input);
  ...
  it('filters out empty and whitespace-only pieces', () => {
    ...
    // The split returns ['', '<<<dotprompt:role:system', ''] but
    // empty/whitespace pieces are filtered out.
    const output = splitByRoleAndHistoryMarkers(input);
  ...
  it('handles adjacent markers correctly', () => {
    ...
    // Splitting yields: ['', '<<<dotprompt:role:user', '',
    // '<<<dotprompt:history', ''] After filtering, we expect both markers
    // (without the trailing ">>>") in the result.
    const output = splitByRoleAndHistoryMarkers(input);
...
describe('splitIntoParts', () => {
  it('should return the entire string in an array if there are no markers', () => {
  ...
  it('should remove parts that are only whitespace', () => {
    ...
    const parts = splitByMediaAndSectionMarkers(source);
    expect(parts).toEqual(['<<<dotprompt:media:url']);
  });
});
