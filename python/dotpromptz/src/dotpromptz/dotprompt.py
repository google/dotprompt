...
# ---------------------------------------------------------------------------
# Simulated external dependencies and helper functions/types
# ---------------------------------------------------------------------------
...
# Dummy implementations for imported functions
...
class Dotprompt:
    ...
    def render(
        ...
        messages = to_messages(rendered, data or {})  # type: ignore
    ...
    def identifyPartials(self, template: str) -> set[str]:
        """
        Identifies partials used within a template.

        Args:
            template: The template string.

        Returns:
            A set of partial names found in the template.
        """
        ...
        class PartialVisitor(Visitor):
            """
            Visitor class to collect partial names from the AST.
            """
    ...
    async def resolvePartials(self, template: str) -> None:
        """
        Resolves and registers partials found in the template.
            ...
            if name not in self.handlebars.partials:
                ...
                if self.partialResolver:
                    ...
                    if asyncio.iscoroutine(result):
                        ...
                        # Static type checker needs help here
        ...
        await asyncio.gather(
            *(resolve_partial(name) for name in list(partials))
        )
...
class Visitor:
    ...
    def accept(self, ast: dict[str, Any]) -> None:
        """
        Accepts an AST and processes its partials if present.

        Args:
            ast: The AST of the template.
        """
        for partial in ast.get('partials', []):
