...
ToolArgument: TypeAlias =  str | ToolDefinition
...
class PromptMetadata(HasMetadata, Generic[T]):
    ...
    Attributes:
      ...
      model: The name of the model to use for this prompt, e.g. `vertexai/gemini-1.0-pro`.
      tools: Names of tools (registered separately) to allow use of in this prompt.
      ...
      raw: This field will contain the raw frontmatter as parsed with no additional processing or substitutions. If your implementation requires custom fields they will be available here.
      ext: Fields that contain a period will be considered "extension fields" in the frontmatter and will be gathered by namespace. For example, `myext.foo: 123` would be available at `parsedPrompt.ext.myext.foo`. Nested namespaces will be flattened, so `myext.foo.bar: 123` would be available at `parsedPrompt.ext["myext.foo"].bar`.
    """
    name: str | None = None
    variant: str | None = None
    version: str | None = None
...
Part: TypeAlias = TextPart | DataPart | MediaPart | ToolRequestPart[Any] | ToolResponsePart[Any] | PendingPart
...
class DataArgument(Generic[T]):
    ...
    Attributes:
        ...
        context: Items in the context argument are exposed as `@` variables, e.g. `context: {state: {...}}` is exposed as `@state`.
    """
    input: T | None = None
...
JsonSchema: TypeAlias = Any

class SchemaResolver(Protocol):
    """Function that can resolve a provided schema name to an underlying JSON schema.
    
    Utilized for shorthand to a schema library provided by an external tool.
    """

    def __call__(self, schemaName: str) -> JsonSchema | None:
        ...

class ToolResolver(Protocol):
    """Function that can resolve a provided tool name to an underlying ToolDefinition. 
    
    Utilized for shorthand to a tool registry provided by an external library.
    """

    def __call__(self, toolName: str) -> ToolDefinition | None:
        ...
...
class RenderedPrompt(PromptMetadata[T]):
    ...
    It includes all of the prompt metadata as well as a set of `messages` to be sent to the  model.
...
class PromptFunction(Protocol, Generic[T]):
    """Function that takes runtime data/context and returns a rendered prompt result.
    """

    prompt: ParsedPrompt[T]

    def __call__(self, data: DataArgument[Any, Any], options: PromptMetadata[T] | None = None) -> RenderedPrompt[T]:
        ...

class PromptRefFunction(Protocol, Generic[T]):
    """Function that takes runtime data / context and returns a rendered prompt result after loading a prompt via reference.
    """
...
class PromptStore(Protocol):
    ...
    def list_partials(self, options: dict[str, Any] | None = None) -> dict[str, Any]:
        """Return a list of partial names available in this store."""
    
    def load(self, name: str, options: dict[str, Any] | None = None) -> PromptData:
        """Retrieve a prompt from the store."""
    
    def load_partial(self, name: str, options: dict[str, Any] | None = None) -> PromptData:
        """Retrieve a partial from the store."""

class PromptStoreWritable(PromptStore, Protocol):
    """PromptStore that also has built-in methods for writing prompts in addition to reading them."""
