...
RESULT_COMBINATIONS = list(itertools.product(('success', 'failure', 'skipped', 'cancelled'), repeat=len(TARGET_KEYS)))
...
def test_all_nonempty_proper_subsets_are_rejected(subset: tuple[str, ...]) -> None:
    with pytest.raises(release_verifier.VerificationError, match=rf'all {len(TARGET_KEYS)}'):
        release_verifier.require_all_targets(subset)


def test_exact_target_set_is_required() -> None:
    ...
    with pytest.raises(release_verifier.VerificationError, match='unknown'):
        release_verifier.require_all_targets((*TARGET_KEYS[:-1], 'freebsd_x86_64'))
...
def test_status_gate_rejects_missing_and_extra_jobs() -> None:
    ...
    results['freebsd_x86_64'] = 'success'
    assert not release_verifier.publish_gate(results)


def test_collect_requires_all_artifact_directories(tmp_path: Path) -> None:
    artifacts = tmp_path / 'artifacts'
...
def test_unsafe_zip_members_are_rejected(tmp_path: Path, member: str) -> None:
    ...
    with pytest.raises(
        ...
        match=r'(unsafe wheel member|RECORD paths do not exactly cover wheel members)',
    ):
        release_verifier.verify_wheel(wheel, target, VERSION)
...
def test_pip_install_on_windows_uses_win_amd64_and_win_arm64_wheels() -> None:
    """Windows pip install gets win_amd64 or win_arm64 from the required set."""
    by_key = {target.key: target for target in TARGETS}
    assert by_key['windows_x86_64'].platform == 'win_amd64'
    assert by_key['windows_arm64'].platform == 'win_arm64'
    names = release_verifier.expected_filenames(VERSION)
    assert len(names) == 8
    assert f'dotpromptz_handlebars-{VERSION}-cp310-abi3-win_amd64.whl' in names
    assert f'dotpromptz_handlebars-{VERSION}-cp310-abi3-win_arm64.whl' in names
