...
def test_publish_workflow_has_no_subset_controls() -> None:
    dispatch = WORKFLOW.split('jobs:', maxsplit=1)[0]
    assert 'enable_' not in dispatch
    assert '#   - Windows x86_64: ENABLED' in WORKFLOW
    assert '#   - Windows ARM64:  ENABLED' in WORKFLOW
    assert '#   - Windows:       UNSUPPORTED' not in WORKFLOW
    assert 'windows-latest' in _job('build_windows_x86_64')
    assert 'windows-latest' in _job('build_windows_arm64')
    assert 'windows-latest' in _job('smoke_test_windows_x86_64')
    assert 'windows-11-arm' in _job('smoke_test_windows_arm64')
...
def test_workflow_builds_and_smokes_windows_x86_64() -> None:
    """Windows x64 pip install is a required build and TestPyPI smoke."""
    build = _job('build_windows_x86_64')
    smoke = _job('smoke_test_windows_x86_64')
    assert 'windows-latest' in build
    assert 'architecture: x64' in build
    assert 'maturin build --release --strip -o dist' in build
    assert 'needs: [setup, testpypi_publish]' in smoke
    assert '--target windows_x86_64' in smoke
    assert '- smoke_test_windows_x86_64' in _job('production_publish')


def test_workflow_builds_and_smokes_windows_arm64() -> None:
    """Windows ARM pip install is a required build and TestPyPI smoke."""
    build = _job('build_windows_arm64')
    smoke = _job('smoke_test_windows_arm64')
    assert 'windows-latest' in build
    assert 'aarch64-pc-windows-msvc' in build
    assert 'needs: [setup, testpypi_publish]' in smoke
    assert 'windows-11-arm' in smoke
    assert '--target windows_arm64' in smoke
    assert '- "3.10"' not in smoke
    assert '- smoke_test_windows_arm64' in _job('production_publish')
...
def test_builds_are_exactly_one_cp310_abi3_wheel_per_target() -> None:
    ...
    assert 'windows-latest' in _job('build_windows_x86_64')
    assert 'aarch64-pc-windows-msvc' in _job('build_windows_arm64')
...
def test_contract_is_poison_detecting_resumable_not_transactional() -> None:
    assert 'poison-detecting, resumable exact-eight publication' in WORKFLOW
