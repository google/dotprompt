...
TARGETS = (
    ...
    Target('windows_x86_64', 'wheels-windows-x86_64-3.10', 'win_amd64'),
    Target('windows_arm64', 'wheels-windows-arm64-3.10', 'win_arm64'),
)
...
def require_all_targets(values: Sequence[str] | None = None) -> frozenset[str]:
    """Reject every publishing target set except the complete native set."""
    ...
    if set(selected) != TARGET_KEYS:
        raise VerificationError(f'publishing requires all {len(TARGETS)} native targets')
    return TARGET_KEYS
...
def publish_gate(results: Mapping[str, str]) -> bool:
    """Accept only exactly the required successful native build or smoke jobs."""
...
def expected_filenames(version: Version) -> frozenset[str]:
    """Return the exact required wheel distribution."""
...
def collect_wheels(artifact_root: Path, version: Version) -> dict[str, Path]:
    ...
    if {entry.name for entry in entries} != {target.artifact for target in TARGETS}:
        raise VerificationError(f'artifact directories do not match all {len(TARGETS)} targets')
...
def manifest_for_dist(dist: Path, version: Version) -> dict[str, str]:
    ...
    if set(paths) != expected_filenames(version) or len(list(dist.iterdir())) != len(paths):
        raise VerificationError(f'dist must contain exactly the {len(TARGETS)} expected wheels')
...
def read_manifest(path: Path, version: Version) -> dict[str, str]:
    """Read and validate one exact-set SHA-256 manifest."""
    try:
    ...
    if set(files) != expected_filenames(version):
        raise VerificationError(f'release manifest must contain exactly {len(TARGETS)} wheels')
    ...
    try:
        if args.command == 'release':
            ...
            if not publish_gate(_parse_results(args.result)):
                raise VerificationError(f'result gate requires {len(TARGETS)} successful jobs')
